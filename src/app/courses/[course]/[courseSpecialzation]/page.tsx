...
const Courses = async ({ params }: any) => {
  const { courseSpecialzation } = params;
  const page = await getCoursesSpecialization(courseSpecialzation);
  const contentSections = page?.data?.[0]?.attributes?.contentSections;
  return contentSections?.map((section: any, index: number) => {
    if (section.__component === "atoms.slider123") {
      return <Slider key={`atoms.slider123 ${index}`} data={section} />;
    } else if (section.__component === "atoms.page-content") {
      return (
        <div
          key={`atoms.page-content ${index}`}
          className="py-12 md:py-16 container mx-auto"
        >
          <RichTextEditor
